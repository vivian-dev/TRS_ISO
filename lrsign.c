...
int lrsign(const unsigned char *sk, const int64_t I, const unsigned char *pks, 
	const int64_t rings, const unsigned char *m, uint64_t mlen, unsigned char *sig, uint64_t *sig_len){
	...
	init_grpelt(z);
	init_grpelt(s);
	sample_S1L(s,sk);   //sk����s

	// compute Tag
	public_key *tag = (public_key *) LRSIG_TAG(sig);
	derive_tag(tag,s);  //T=s*T_0

	// choose salt
	RAND_bytes(LRSIG_SALT(sig),HASH_BYTES);  //1. sig<--(salt)
	...
	// pick random seeds
	restart: generate_seed_tree(seed_tree,EXECUTIONS,LRSIG_SALT(sig));//��salt(sig)����������seed_tree(seed1,...seedM)

	// hash message
	HASH(m,mlen,FM_MESSAGE_HASH(fm));//fmΪsalt+��Ϣ�Ĺ�ϣֵ=(salt||H(m))
	// printf("H(M)\n");
	// for(int i=0;i<HASH_BYTES;i++)
	// {
	// 	printf("%02x",FM_MESSAGE_HASH(fm)[i]);
	// }
	// printf("\n");
	//line3: P_1((R,T),(sk,I))-->(salt,(com_1,...com_M))
	for (int i = 0; i < EXECUTIONS; ++i)  //EXECUTIONS=M
	{
		...
		(*ctr)  = EXECUTIONS + i; 
		//seed_i(seedbuf)-->(r,bits1,...bitsN)(expand_buf)
		...
		PREP_GRPELT pg;
		do_half_action(&pg,r[i]);  //pg=r[i]

		// Memory access at secret location!
		memcpy(commitment_randomness + i*SEED_BYTES , expand_buf + I*SEED_BYTES , SEED_BYTES);//bits_I

		// compute and commit to T'
		do_tag_action(&Tprime,tag,r[i]); //Tprime=r[i]*tag  r[i] in S2
		commit(&Tprime, zero_seed , RSIG_SALT(sig), FM_TPRIME(fm) + i*HASH_BYTES);//��T'(Tprime)��ŵ��(Tprime||zero_seed||sig)--->fms

		// compute R_i and commitments
		for (int j = 0; j < rings; ++j)  //base OR, rings=N
		{
			//commitments=(C1,...CN)
			finish_action(&R,(public_key*) (pks + j*sizeof(public_key)), &pg); //R_i=r*X_i
			commit(&R,expand_buf + j*SEED_BYTES, RSIG_SALT(sig), commitments + j*HASH_BYTES); //��R_i��ŵ��(R_i||expand_buf||sig)--->commitments
		}
		...
		build_tree_and_path(commitments, logN, I, FM_ROOTS(fm) + i*HASH_BYTES, paths + i*HASH_BYTES*logN );//(commitments)-->(root,tree)
	}

	//line4: generate challenge  
	EXPAND(fm, FM_BYTES, LRSIG_CHALLENGE(sig), SEED_BYTES);//root(fm)-->sig
	unsigned char challenge[EXECUTIONS];
	derive_challenge(LRSIG_CHALLENGE(sig),challenge);//sig-->challenge

	//line5:((sk,I),chall)-->rsp
	int zeros = 0;
	int ones = 0;
	for (int i = 0; i < EXECUTIONS; ++i)
	{
	...
	release_seeds(seed_tree, EXECUTIONS, challenge, LRSIG_SEEDS(sig,logN) , sig_len );//seed_internal
	(*sig_len) *= SEED_BYTES;
